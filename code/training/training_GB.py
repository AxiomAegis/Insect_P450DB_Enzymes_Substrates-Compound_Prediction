...
# 获取当前时间并格式化
current_time_formatted = datetime.now().strftime("%Y-%m-%d") # %Y-%m-%d %H:%M:%S
...
def get_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The output directory where the xgb model parameters will be written.",
    )    
    parser.add_argument(
        "--save_pred_path",
        type=str,
...
create_dir_not_exist(args.save_xgb_path)

setting = "logs/" + current_time_formatted + "_" + args.log_name + '_gpus' + str(n_gpus) +'_layers' + str(args.num_hidden_layers) +'_xgboost_training'
...
def trainer(gpu, args, device):
    logging.info(current_time_formatted)
    logging.info(args)
    ...
    # yb添加的
    def get_predictions(param, dM_train, dM_val, save_xgb_name):
        param, num_round, dM_train = set_param_values_V2(param = param, dtrain = dM_train)
        bst = xgb.train(param,  dM_train, num_round)
        # 保存模型
        logging.info(f'xgb模型保存路径：{args.save_xgb_path}/{save_xgb_name}')
        # 使用二进制写入模式打开文件，并序列化对象
        with open(f'{args.save_xgb_path}/{save_xgb_name}.pkl', 'wb') as file:
            pickle.dump(bst, file)   
        # 加载保存的序列化模型    
        with open(f'{args.save_xgb_path}/{save_xgb_name}.pkl', 'rb') as file:
            load_bst = pickle.load(file)           
        # bst.save_model(f'{args.save_xgb_path}/{save_xgb_name}.json')
        load_y_val_pred = load_bst.predict(dM_val)
        logging.info(f'load_y_val_pred: >>>{load_y_val_pred}')
        y_val_pred = bst.predict(dM_val)
        logging.info(f'y_val_pred: >>>{y_val_pred}')
        
        return(load_y_val_pred)
    ...
    # 打印最优参数
    # logging.info("fmin ESM1b+ChemBERTa2最优参数: ", best)
    # logging.info("trials.argmin ESM1b+ChemBERTa2最优参数: ", trials.argmin)
    # with open(f'{args.save_xgb_path}/fmin_ESM1b+ChemBERTa2最优参数.json', 'w') as f:
    #     json.dump(best, f)
    # with open(f'{args.save_xgb_path}/trials.argmin_ESM1b+ChemBERTa2最优参数.json', 'w') as f:
    #     json.dump(trials.argmin, f)

    #predictions for validation and test set on test set:
    logging.info("ESM1b+ChemBERTa2")
    logging.info("Validation set:")
    y_val_pred_all = get_predictions(param = trials.argmin, dM_train = dtrain, dM_val = dvalid, save_xgb_name = "y_val_pred_all")
    get_performance_metrics(pred = y_val_pred_all, true = val_labels)
    logging.info("Test set:")
    y_test_pred_all = get_predictions(param = trials.argmin, dM_train = dtrain_val, dM_val = dtest, save_xgb_name = "ESM1b_ChemBERTa")
    ...
    best = fmin(fn = train_xgboost_model_all, space = space_gradient_boosting,
        ...
        # 打印最优参数
    # logging.info("fmin ESM1b+ChemBERTa +cls最优参数: ", best)
    # logging.info("trials.argmin ESM1b+ChemBERTa +cls最优参数: ", trials.argmin)
    # with open(f'{args.save_xgb_path}/fmin_ESM1b+ChemBERTa+cls最优参数.json', 'w') as f:
    #     json.dump(best, f)
    # with open(f'{args.save_xgb_path}/trials.argmin_ESM1b+ChemBERTa+cls最优参数.json', 'w') as f:
    #     json.dump(trials.argmin, f)    

    #predictions for validation and test set on test set
    logging.info("ESM1b+ChemBERTa2+cls-token")
    logging.info("Validation set:")
    y_val_pred_all_cls = get_predictions(param = trials.argmin, dM_train = dtrain_all_cls, dM_val = dvalid_all_cls, save_xgb_name = "y_val_pred_all_cls")
    get_performance_metrics(pred = y_val_pred_all_cls, true = val_labels)
    logging.info("Test set:")
    y_test_pred_all_cls = get_predictions(param  = trials.argmin, dM_train = dtrain_val_all_cls, dM_val = dtest_all_cls, save_xgb_name = "ESM1b_ChemBERTa_cls")
    ...
    best = fmin(fn = train_xgboost_model_cls, space = space_gradient_boosting,
        ...
        # 打印最优参数
    # logging.info("fmin cls token最优参数: ", best)
    # logging.info("trials.argmin cls token最优参数: ", trials.argmin)
    # with open(f'{args.save_xgb_path}/fmin_cls-token最优参数.json', 'w') as f:
    #     json.dump(best, f)
    # with open(f'{args.save_xgb_path}/trials.argmin_cls-token最优参数.json', 'w') as f:
    #     json.dump(trials.argmin, f)      
    ...
    logging.info("Validation set:")
    y_val_pred_cls = get_predictions(param = trials.argmin, dM_train = dtrain_cls, dM_val = dvalid_cls, save_xgb_name = "y_val_pred_cls")
    get_performance_metrics(pred = y_val_pred_cls, true = val_labels)
    logging.info("Test set:")
    y_test_pred_cls = get_predictions(param = trials.argmin, dM_train = dtrain_val_cls, dM_val = dtest_cls, save_xgb_name = "cls")
