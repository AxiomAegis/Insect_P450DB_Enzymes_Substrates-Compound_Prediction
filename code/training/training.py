...
def get_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The prefix name of the model.",
    )    
    # debug报错 yb
    parser.add_argument(
        "--pretrained_model",
...
# create_dir_not_exist(f'{args.embed_path}')
...
setting = str(n_gpus) +'gpus_bs' + str(eff_bs) +'_'+str(args.learning_rate) +'_layers' + str(args.num_hidden_layers)

logger = logging.getLogger()
logger.setLevel(logging.INFO)
logging.basicConfig(format='%(levelname)s:%(message)s', level=logging.INFO)
fhandler = logging.FileHandler(filename= f'logs/{current_time_formatted}_{setting}.txt', mode='a')
logger.addHandler(fhandler)

print(os.path.join(args.save_model_path, f'{args.model_prefix}_{setting}.pkl'))
...
def evaluate(args, model, valloader, criterion, device, gpu):
    ...
    with torch.no_grad():
        for step, batch in enumerate(valloader):
            ...
            if args.binary_task:
                y_true.extend(labels.cpu().bool().numpy().copy())
                y_pred.extend(preds.cpu().numpy().copy())
                # logging.info(f">>>测试preds: {type(preds.cpu()), preds.cpu()}")
            else:
                y_true.extend(labels.cpu().numpy().copy())
                y_pred.extend(preds.cpu().numpy().copy())
        ...
        if not args.binary_task:
            ...
            y_true = np.array(y_true).astype(int).copy()
            # debug yb
            # y_pred = [tensor.numpy() for tensor in y_pred]
            # logging.info(f">>>测试报错type y_pred 1: {type(y_pred), y_pred}")
            y_pred = np.array(y_pred).copy()
            # logging.info(f">>>测试报错type y_pred 2: {type(y_pred), y_pred}")
            auc_score = roc_auc_score(y_true, y_pred)
            # logging.info(f">>>测试报错type y_pred 3: {type(y_pred), y_pred}")
            y_pred = np.round(y_pred).astype(int).copy()
            # logging.info(f">>>测试报错type y_pred 4: {type(y_pred), y_pred}")
    ...
    # stat = top_stats[0]
    # console.print("%s memory blocks: [bold blue]%.1f KiB[/bold blue]" % (stat.count, stat.size / 1024))
...
def trainer(gpu, args, device):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        trainloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=0, sampler=trainsampler, pin_memory=True)
        valloader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=0, sampler=valsampler, pin_memory=True)
        ...
        if val_loss < best_val_loss:
            ...
            if is_main_process():
                torch.save(model.state_dict(), os.path.join(args.save_model_path, f'{args.model_prefix}_{setting}.pkl'))
