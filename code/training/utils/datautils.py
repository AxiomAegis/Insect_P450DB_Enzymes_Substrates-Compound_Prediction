...
class SMILESProteinDataset(Dataset):
    ...
    def __len__(self):
        # print(f"self.total_datacount: {self.total_datacount}")
        return self.total_datacount

    def __getitem__(self, idx):
        # logging.info(f"1：   Item idx: {idx}, len(help_df): {len(self.mappings)}")
        start_time = time()
        ...
        # logging.info(f"2：   Item idx: {idx}, len(help_df): {len(self.mappings)}, prev_subset_max_idx : {prev_subset_max_idx}, curr_subset_max_idx : {curr_subset_max_idx}")
        ...
        # logging.info(f"3：   Item idx: {idx}, len(help_df): {len(self.mappings)}, prev_subset_max_idx : {prev_subset_max_idx}, curr_subset_max_idx : {curr_subset_max_idx}")
        ...
        # 修复内存泄露 yb
        # print("dataloader数据类型", type(smiles_emb), type(smiles_attn_mask), type(protein_emb), type(protein_attn_mask), type(labels), type(index))
