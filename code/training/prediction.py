...
# 获取当前时间并格式化
current_time_formatted = datetime.now().strftime("%Y-%m-%d")
...
def trainer(gpu, args, device):
    ...
    testsampler = DistributedSampler(test_dataset, shuffle = False, num_replicas = args.world_size, rank = gpu)
    ...
    def get_predictions(save_xgb_name, dM_val):
        # 加载保存的序列化模型    
        with open(f'{args.xgb_path}/{save_xgb_name}.pkl', 'rb') as file:
            load_bst = pickle.load(file)    
        y_val_pred = load_bst.predict(dM_val)
        return(y_val_pred)

    ############# ESM1b+ChemBERTa2
    
    test_X_all = np.concatenate([test_esm1b, test_smiles], axis = 1)
    logging.info(f"test_X_all shape: {test_X_all.shape}")
    ...
    y_test_pred_all = get_predictions(save_xgb_name = "ESM1b_ChemBERTa", dM_val = dtest)
    ...
    logging.info("Test set:")
    y_test_pred_all_cls = get_predictions(save_xgb_name = "ESM1b_ChemBERTa_cls", dM_val = dtest_all_cls)



    ############# cls token
    logging.info(f"test_cls shape: {test_cls.shape}")
    dtest_cls = xgb.DMatrix(np.array(test_cls), label = np.array(test_labels).astype(float))
                
    #predictions for validation and test set on test set:
    logging.info("cls-token")
    logging.info("Test set:")
    y_test_pred_cls = get_predictions(save_xgb_name = "cls", dM_val = dtest_cls)
    ...
    #############
    # ESM1b+ChemBERTa2+cls: 0.24, ESM1b+ChemBERTa2: 0.66, cls-token: 0.09999999999999998
    best_i, best_j, best_k = 0.24, 0.66, 0.1
    ...
    if args.save_pred_path != "":
        ...
        for k, ind in enumerate(test_indices):
            test_df["y_pred"][ind] = y_test_pred[k]

        # 假设你想把列'A'移到最后一列
        column_to_move = 'output'
        test_df = test_df[[col for col in test_df if col != column_to_move] + [column_to_move]]
        test_df.to_csv(join(args.save_pred_path, "final_test_pred.csv"), index=None)
